...
import {
  ...
  Loader2, Zap, Maximize2, Minimize2,
} from "lucide-react";
...
function ChatUI({
  ...
  const suggestions = locale === "ru" ? SUGGESTIONS.ru : SUGGESTIONS.en;
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p style={{ color: "#34d399", fontSize: 11, fontFamily: "monospace" }}>
              {locale === "ru" ? "● онлайн · GPT-4o mini" : "● online · GPT-4o mini"}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          {messages.length > 0 && (
            <button
              ...
              title={locale === "ru" ? "Очистить" : "Clear"}
            >
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5" style={{ background: "#070810" }}>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center px-4 space-y-6">
            ...
            <div>
              <p style={{ color: "#fff", fontWeight: 800, fontSize: 20, fontFamily: "var(--font-syne)", lineHeight: 1.2, marginBottom: 8 }}>
                {locale === "ru" ? "Привет! Я Vibe AI" : "Hey! I'm Vibe AI"}
              </p>
              <p style={{ color: "#6b7280", fontSize: 13, lineHeight: 1.6, maxWidth: 260, margin: "0 auto" }}>
                {locale === "ru"
                  ? "Эксперт по ИИ, Vibe Coding и промпт-инженерии. Спроси что угодно!"
        ...
        {/* Messages */}
        {messages.map((msg: { id: string; role: "user" | "assistant"; content: string; createdAt: number }, i: number) => (
          <div
            key={msg.id}
      ...
      >
        ...
        >
          <textarea
            ...
            placeholder={locale === "ru" ? "Спроси об ИИ, Vibe Coding..." : "Ask about AI, Vibe Coding..."}
            rows={1}
            style={{
              flex: 1,
          ...
          <button
            ...
            style={{
              ...
              boxShadow: input.trim() && !isLoading ? "0 2px 12px rgba(16,185,129,0.35)" : "none",
            }}
          >
            {isLoading
              ? <Loader2 style={{ width: 14, height: 14, color: "#6b7280", animation: "spin 1s linear infinite" }} />
              : <Send style={{ width: 14, height: 14, color: input.trim() ? "#fff" : "#4b5563" }} />}
          </button>
        </div>
        <p style={{ textAlign: "center", color: "#374151", fontSize: 10, marginTop: 8 }}>
          Powered by GPT-4o mini · Enter ↵ to send
        </p>
      </div>
    </div>
  );
...
export function ChatAssistant({ userContext }: ChatAssistantProps) {
  ...
  const {
    ...
    clearMessages, setMessages, setOpen, setLoading, setHistoryLoaded,
  } = useChatStore();
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    addMessage({ role: "user", content: trimmed }, userMsgId);
    ...
    try {
      ...
      updateLastAssistant(locale === "ru"
        ? "Ошибка соединения. Проверь интернет."
        : "Connection error. Check your internet.");
    } finally {
      setLoading(false);
      isStreaming.current = false;
    }
  }, [messages, locale, userContext, addMessage, updateLastAssistant, setLoading]);
